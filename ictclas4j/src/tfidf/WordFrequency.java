...
public class WordFrequency {
	...
	public void countWordFrequency2() throws FileNotFoundException {
	 ...
	 * 用于主算权重
	 * 
	 * keywordsNo 1--subject的权重 2--action的权重 3-- 4--
	 * 
	 * 范尼斯特鲁伊禁区内射门
	 * 
	 * wordInSentence 词i（射门）在句子j中出现的次数------------------1
	 * 
	 * allWordInSentence 句子j中所有词的个数-----------------------------4
	 */
	public double countWordFrequency(int keywordsNo, int wordInSentence,
			int allWordInSentence) throws FileNotFoundException {
		GetKeywords keywords = new GetKeywords();
		/*
		 * 运行keywords.GetKeywords提供的分词和关键词提取模块
		 */
		 ...
		 */
		for (int j = 0; j < 4; j++) {
			idf[j] = Math.log(((double) (item[0].allSentencesNo))
					/ (double) sentenceWithWord[j]);
		}
		/*
		 * w=tf*idf;
		 */
	...
	public double[] countWordFrequency() throws FileNotFoundException {
		GetKeywords keywords = new GetKeywords();
		/*
		 * 运行keywords.GetKeywords提供的分词和关键词提取模块
		 */
		...
		for (int j = 0; j < 4; j++) {
			...
			System.out.println("sentenceWithWord["+j+"]:"+sentenceWithWord[j]);
		}
		/*
		 * 计算idf
		 */
	...
	public static void main(String args[]) throws IOException {
		...
		for (int k = 0; k < 4; k++) {
			System.out.println("sentenceWithWord[]:"
					+ wordFrequency.sentenceWithWord[k]);
